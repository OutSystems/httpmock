...
var optimist = require('optimist')
	...
	.string('c').alias('c', 'config').describe('Configuration file for the responses.')
	.default('p', 8888).alias('p', 'port')
...
var configFile = argv.config ? path.resolve(argv.config) : path.resolve(__dirname, '../baseConfig.json');
...
function normalizeRule(configRule) {
	...
	configRule.ignoredHeaders = configRule.ignoredHeaders || defaultIgnoredHeaders;
...
var defaultRule = { "status": 200, response: "No rule was matched.", urlFilter: "" };
...
function getMatchingConfigRule(requestUrl, method) {
	...
	for (var i = 0; i < configArray.length; i++) {
		...
		if (configRule.method === "" || configRule.method === method) {
			// Match Url
			if (new RegExp('^/?' + configRule.urlFilter + '$').test(requestUrlNoQueryString)) {
				return configRule;
			}
		}
	}
...
function shouldFilterHeader(headerName, requestUrl, requestMethod) {
	...
	var isToIgnore = !matchingRule.ignoredHeaders.every(function(e) { return e != headerName });

	return (headerName == 'host') || (headerName == 'connection') || (headerName == 'expect') || isToIgnore;
}

function filterHeaders(headers, requestUrl, requestMethod) {
	var filtered = {};
	Object.keys(headers).forEach(function (key) {
		if (!(shouldFilterHeader(key, requestUrl, requestMethod) || (key == 'content-length' && headers[key] == '0'))) {
...
function normalizeData(data) {
	...
	data = data.replace(/%[a-fA-F0-9]{2}/g, function(match) { return match.toLowerCase() });
	...
	request.on('end', function() {
		...
		if (matchingRule.response !== "") {
			if (matchingRule.encoding === "") {
				response.write(matchingRule.response);
			} else {
				response.write(iconv.encode(matchingRule.response, matchingRule.encoding), 'binary');
			}
		}
		response.end();
	});
...
server.on('error', function (e) {
  if (e.code == 'EADDRINUSE') {
	...
	if (retryBindCount <= maxRetryBindCount) {
		...
		setTimeout(function () { server.listen(argv.p); }, 500);
	} else {
